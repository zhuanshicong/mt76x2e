...
#ifdef TXBF_SUPPORT
	BOOLEAN bITxBf_Cal; /* When DUT receives a sounding request, it will send a null packet after MAC HW sends a sounding response */
...
//MPS
	BOOLEAN MPS_Start;
	UINT MPS_iCount;
	UCHAR MPS_SeqData[2048];
	UCHAR MPS_PayloadLen[2048];
	UCHAR MPS_PacketCount[2048];
	UCHAR MPS_PowerGain[2048];
} ATE_INFO, *PATE_INFO;
...
VOID RtmpSoundingDmaEnable(
	IN PRTMP_ADAPTER pAd,
	IN INT Enable);

INT ATESetUpSoundingFrame(
	IN PRTMP_ADAPTER pAd,
	IN UINT32 TxIdx);
