	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if ((!IS_RT8592(pAd)) && (!IS_MT7610(pAd)) &&(!IS_MT76x2(pAd)) && (!IS_MT7601(pAd)) && (!IS_RT6352(pAd)))
		BbpSoftReset(pAd);
	...
	/*Reinitialize Tx Ring before Tx DMA is enabled.*/
	for (ring_index = 0; ring_index < TX_RING_SIZE; ring_index++)
	{
#ifdef RT_BIG_ENDIAN
		pDestTxD = (PTXD_STRUC) pAd->TxRing[0].Cell[ring_index].AllocVa;
		NdisMoveMemory(&tx_hw_info[0], pDestTxD, TXD_SIZE);
		pTxD = (TXD_STRUC *)&tx_hw_info[0];
		RTMPDescriptorEndianChange((PUCHAR)pTxD, TYPE_TXD);
#else
		/* Point to Tx indexed tx ring descriptor */
		pTxD = (PTXD_STRUC) pAd->TxRing[0].Cell[ring_index].AllocVa;
#endif /* RT_BIG_ENDIAN */
...
#ifdef RTMP_INTERNAL_TX_ALC
#ifdef RT6352
	if (IS_RT6352(pAd) && (pATEInfo->bAutoTxAlc == TRUE))
	{
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	for (ring_index = 0; (ring_index < TX_RING_SIZE-1) && (ring_index < pATEInfo->TxCount); ring_index++)
	{
		...
		{
			if (ATESetUpNDPAFrame(pAd, TxIdx) != 0)
				return NDIS_STATUS_FAILURE;
		}
		else
		{
	...
	if (pATEInfo->TxWI.TXWI_N.PHYMODE == MODE_VHT && pATEInfo->bTxBF == TRUE && pATEInfo->txSoundingMode != 0)
	{
		ATESetUpNDPAFrame(pAd, pTxRing->TxCpuIdx);
	}
	else
	{
	ATESetUpFrame(pAd, pTxRing->TxCpuIdx);
...
#ifdef TXBF_SUPPORT
	pAd->ate.bITxBf_Cal = TRUE; // When Golden receives a sounding request, it will send a null packet after MAC HW sends a sounding response
#endif 
	...
	IN	PSTRING			arg)
{
	...
	{
		DBGPRINT_ERR(("Set_ATE_CHANNEL_Proc::%d Out of range, it should be in range of 1~14.\n",channel));
		return FALSE;
	}
...
INT Set_ATE_AUTO_RESPONDER_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING 		arg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
	...
	auto_respond = simple_strtol(arg, 0, 10);
	
	DBGPRINT(RT_DEBUG_OFF,("%s::value=%d\n", __FUNCTION__, auto_respond));
	if (auto_respond > 0)
	{
		DBGPRINT(RT_DEBUG_TRACE,("Enable Auto Responder\n"));
		RTMP_IO_READ32(pAd, AUTO_RSP_CFG, &MacValue);
		MacValue = MacValue | 0x00000001;
		RTMP_IO_WRITE32(pAd, AUTO_RSP_CFG, MacValue);
		/* set XIFS_TIME_CFG to align normal driver to gurantee noraml ack behavior */
		RTMP_IO_WRITE32(pAd, XIFS_TIME_CFG, 0x33a40d0a);

	} else if (auto_respond == 0){
		DBGPRINT(RT_DEBUG_TRACE,("Disable Auto Responder\n"));
		RTMP_IO_READ32(pAd, AUTO_RSP_CFG, &MacValue);
		MacValue = MacValue & 0xFFFFFFFE;
		RTMP_IO_WRITE32(pAd, AUTO_RSP_CFG, MacValue);
		AsicSetDevMac(pAd, pAd->CurrentAddress);
	}
	
	DBGPRINT(RT_DEBUG_TRACE, ("Ralink: Set_ATE_AUTO_RESPONDER_Proc success\n"));
	...
	IN	PSTRING			arg)
{
	...
	for (i = 0; i < calLoop; i++)
	{
		...
		RTMP_IO_READ32(pAd, TXBE_R12, &value);
		RTMP_IO_WRITE32(pAd, TXBE_R12, value & (~0x28));
...
==========================================================================
*/
INT Set_ATE_TXBF_Gd_Verify_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING			arg)
{
	UCHAR ch;
	...
	IN	PSTRING			arg)
{
	...
	for (i = 0; i < calLoop; i++)
	{
		...
		ITxBFPhaseCalibrationStartUp(pAd, 1, TRUE, ch);
		ITxBFPhaseCalibrationStartUp(pAd, 1, TRUE, ch);
		ITxBFPhaseCalibrationStartUp(pAd, 1, TRUE, ch);
		ITxBFPhaseCalibrationStartUp(pAd, 1, TRUE, ch);
		ITxBFPhaseCalibrationStartUp(pAd, 1, TRUE, ch);
...
INT Set_ATE_TXBF_Intloop_Phase_Verify_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN  PSTRING         arg)
{
	UCHAR ch, calLoop, i;
	...
	for (i = 0; i < calLoop; i++)
	{
		...
		ITxBFPhaseCalibrationStartUp(pAd, 0, FALSE, ch);
	}
...
INT Set_ATE_TXBF_Intloop_Phase_Cal_and_Verify_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN  PSTRING         arg)
{
	UCHAR ch, calLoop, i;
	...
	for (i = 0; i < calLoop; i++)
	{
		...
		RTMP_IO_WRITE32(pAd, TXBE_R13, 0);  // for method1

		/* Phase calibration process */
		// LNA caliulation
		if (ch > 14)
			pAd->chipOps.fITxBfLNACalibration(pAd, 1, 5, FALSE);
		else
			pAd->chipOps.fITxBfLNACalibration(pAd, 1, 5, TRUE);

		// Tx phase calibration
		ITxBFPhaseCalibrationStartUp(pAd, 1, TRUE, ch);

		/* The accuracy verification of phase calibration */
		// LNA compensation
		pAd->chipOps.fITxBfLNAPhaseCompensate(pAd);
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	if (IS_MT76x2(pAd))
	{
		...
		pATEInfo->Addr2[0] = 0x00;
		pATEInfo->Addr2[1] = 0x0C;
		pATEInfo->Addr2[2] = 0x43;
		pATEInfo->Addr2[3] = 0x37;
		pATEInfo->Addr2[4] = 0x00;
		pATEInfo->Addr2[5] = 0x00;
		
		pATEInfo->Addr3[0] = 0x00;
		pATEInfo->Addr3[1] = 0x0A;
		pATEInfo->Addr3[2] = 0x40;
		pATEInfo->Addr3[3] = 0xAA;
		pATEInfo->Addr3[4] = 0xBB;
		pATEInfo->Addr3[5] = 0xCC;
